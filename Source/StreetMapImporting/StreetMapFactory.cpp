...
bool UStreetMapFactory::LoadFromOpenStreetMapXMLFile( UStreetMap* StreetMap, FString& OSMFilePath, const bool bIsFilePathActuallyTextBuffer, FFeedbackContext* FeedbackContext )
{
	...
	{
		FOSMFile::EOSMWayType RoadType = FOSMFile::EOSMWayType::Other;
		switch( OSMWay.WayType )
		{
			case FOSMFile::EOSMWayType::Bridge:
			case FOSMFile::EOSMWayType::Office:
			case FOSMFile::EOSMWayType::House:
			case FOSMFile::EOSMWayType::Commercial:
			case FOSMFile::EOSMWayType::Kiosk:
			case FOSMFile::EOSMWayType::Warehouse:
			case FOSMFile::EOSMWayType::Supermarket:
			case FOSMFile::EOSMWayType::Church:
			case FOSMFile::EOSMWayType::Shed:
			case FOSMFile::EOSMWayType::Roof:
			case FOSMFile::EOSMWayType::Entrance:
			case FOSMFile::EOSMWayType::Building:
				RoadType = FOSMFile::EOSMWayType::Building;
				break;
		}

		if( RoadType == FOSMFile::EOSMWayType::Building )
		{
	...
	// Adds a barrier to the street map using the OpenStreetMap data, flattening the road's coordinates into our map's space
	auto AddBarrierForWay = [ConvertLatLongToMetersRelative, OSMToCentimetersScaleFactor]( 
		const FOSMFile& OSMFile, 
		UStreetMap& StreetMapRef, 
		const FOSMFile::FOSMWayInfo& OSMWay ) -> bool
	{
		...
		switch( OSMWay.WayType )
		{
			case FOSMFile::EOSMWayType::Bollard:
			case FOSMFile::EOSMWayType::Fence:
			case FOSMFile::EOSMWayType::Hedge:
			case FOSMFile::EOSMWayType::Kerb:
			case FOSMFile::EOSMWayType::Wall:
			case FOSMFile::EOSMWayType::Barrier:
				RoadType = FOSMFile::EOSMWayType::Barrier;
				break;
		}

		if( RoadType != FOSMFile::EOSMWayType::Other )
		{
			// Require at least three points so that we don't have degenerate polygon!
			if( OSMWay.Nodes.Num() > 0 )
			{
				// Create a entry for this way
				FStreetMapBarrier& NewBarrier = *new( StreetMapRef.Barriers )FStreetMapBarrier();

				FVector2D BoundsMin( TNumericLimits<float>::Max(), TNumericLimits<float>::Max() );
				FVector2D BoundsMax( TNumericLimits<float>::Lowest(), TNumericLimits<float>::Lowest() );

				NewBarrier.BuildingPoints.AddUninitialized( OSMWay.Nodes.Num() );
				int32 CurBuildingPoint = 0;
				...
				for( const FOSMFile::FOSMNodeInfo* OSMNodePtr : OSMWay.Nodes )
				{
					...
					// Fill in the points
					NewBarrier.BuildingPoints[ CurBuildingPoint++ ] = NodePos;
				}

				// Make sure the building ended up with a closed polygon, then remove the final (redundant) point
				const bool bIsClosed = NewBarrier.BuildingPoints[ 0 ].Equals( NewBarrier.BuildingPoints[ NewBarrier.BuildingPoints.Num() - 1 ], KINDA_SMALL_NUMBER );
				if( bIsClosed )
				{
					// Remove the final redundant point
					NewBarrier.BuildingPoints.Pop();
				}
				else
				{
					// Wasn't expecting to have an unclosed shape.  Our tolerances might be off, or the data was malformed.
				...
				NewBarrier.BuildingName = OSMWay.Name;
				if( NewBarrier.BuildingName.IsEmpty() )
				{
					NewBarrier.BuildingName = OSMWay.Ref;
				}

				NewBarrier.Height = OSMWay.Height * OSMToCentimetersScaleFactor;
				NewBarrier.BuildingLevels = OSMWay.BuildingLevels;

				NewBarrier.Type = OSMWay.MType;
				NewBarrier.BoundsMin = BoundsMin;
				NewBarrier.BoundsMax = BoundsMax;
	...
	// Adds a amenity to the street map using the OpenStreetMap data, flattening the road's coordinates into our map's space
	auto AddAmenityForWay = [ConvertLatLongToMetersRelative, OSMToCentimetersScaleFactor]( 
		const FOSMFile& OSMFile, 
		UStreetMap& StreetMapRef, 
		const FOSMFile::FOSMWayInfo& OSMWay ) -> bool
	{
		...
		switch( OSMWay.WayType )
		{
			case FOSMFile::EOSMWayType::PostBox:
			case FOSMFile::EOSMWayType::WasteBasket:
			case FOSMFile::EOSMWayType::WasteDisposal:
			case FOSMFile::EOSMWayType::Recycling:
			case FOSMFile::EOSMWayType::Vending:
			case FOSMFile::EOSMWayType::BicycleParking:
			case FOSMFile::EOSMWayType::BusStop:
			case FOSMFile::EOSMWayType::Fuel:
			case FOSMFile::EOSMWayType::Parking:
			case FOSMFile::EOSMWayType::Taxi:
			case FOSMFile::EOSMWayType::ATM:
			case FOSMFile::EOSMWayType::Pharmacy:
			case FOSMFile::EOSMWayType::Clock:
			case FOSMFile::EOSMWayType::Toilet:
			case FOSMFile::EOSMWayType::Amenity:
				RoadType = FOSMFile::EOSMWayType::Amenity;
				break;
		}
		...
		if( RoadType != FOSMFile::EOSMWayType::Other )
		{
			...
			{
				// Create a entry for this way
				FStreetMapAmenity& NewAmenity = *new( StreetMapRef.Amenities )FStreetMapAmenity();

				FVector2D BoundsMin( TNumericLimits<float>::Max(), TNumericLimits<float>::Max() );
				FVector2D BoundsMax( TNumericLimits<float>::Lowest(), TNumericLimits<float>::Lowest() );

				NewAmenity.BuildingPoints.AddUninitialized( OSMWay.Nodes.Num() );
				int32 CurBuildingPoint = 0;
				...
				for( const FOSMFile::FOSMNodeInfo* OSMNodePtr : OSMWay.Nodes )
				{
					...
					// Fill in the points
					NewAmenity.BuildingPoints[ CurBuildingPoint++ ] = NodePos;
				}

				// Make sure the building ended up with a closed polygon, then remove the final (redundant) point
				const bool bIsClosed = NewAmenity.BuildingPoints[ 0 ].Equals( NewAmenity.BuildingPoints[ NewAmenity.BuildingPoints.Num() - 1 ], KINDA_SMALL_NUMBER );
				if( bIsClosed )
				{
					// Remove the final redundant point
					NewAmenity.BuildingPoints.Pop();
				}
				else
				{
					// Wasn't expecting to have an unclosed shape.  Our tolerances might be off, or the data was malformed.
				...
				NewAmenity.BuildingName = OSMWay.Name;
				if( NewAmenity.BuildingName.IsEmpty() )
				{
					NewAmenity.BuildingName = OSMWay.Ref;
				}

				NewAmenity.Height = OSMWay.Height * OSMToCentimetersScaleFactor;
				NewAmenity.BuildingLevels = OSMWay.BuildingLevels;

				NewAmenity.Type = OSMWay.MType;
				NewAmenity.BoundsMin = BoundsMin;
				NewAmenity.BoundsMax = BoundsMax;
	...
	// Adds a amenity to the street map using the OpenStreetMap data, flattening the road's coordinates into our map's space
	auto AddTreeForWay = [ConvertLatLongToMetersRelative, OSMToCentimetersScaleFactor]( 
		const FOSMFile& OSMFile, 
		UStreetMap& StreetMapRef, 
		const FOSMFile::FOSMWayInfo& OSMWay ) -> bool
	{
		...
		switch( OSMWay.WayType )
		{
			case FOSMFile::EOSMWayType::Natural:
			case FOSMFile::EOSMWayType::Tree:
				RoadType = FOSMFile::EOSMWayType::Tree;
				break;
		}
		...
		if( RoadType != FOSMFile::EOSMWayType::Other )
		{
			...
			{
				// Create a entry for this way
				FStreetMapTree& NewTree = *new( StreetMapRef.Trees )FStreetMapTree();

				FVector2D BoundsMin( TNumericLimits<float>::Max(), TNumericLimits<float>::Max() );
				FVector2D BoundsMax( TNumericLimits<float>::Lowest(), TNumericLimits<float>::Lowest() );

				NewTree.BuildingPoints.AddUninitialized( OSMWay.Nodes.Num() );
				int32 CurBuildingPoint = 0;
				...
				for( const FOSMFile::FOSMNodeInfo* OSMNodePtr : OSMWay.Nodes )
				{
					...
					// Fill in the points
					NewTree.BuildingPoints[ CurBuildingPoint++ ] = NodePos;
				}

				// Make sure the building ended up with a closed polygon, then remove the final (redundant) point
				const bool bIsClosed = NewTree.BuildingPoints[ 0 ].Equals( NewTree.BuildingPoints[ NewTree.BuildingPoints.Num() - 1 ], KINDA_SMALL_NUMBER );
				if( bIsClosed )
				{
					// Remove the final redundant point
					NewTree.BuildingPoints.Pop();
				}
				else
				{
					// Wasn't expecting to have an unclosed shape.  Our tolerances might be off, or the data was malformed.
				...
				NewTree.BuildingName = OSMWay.Name;
				if( NewTree.BuildingName.IsEmpty() )
				{
					NewTree.BuildingName = OSMWay.Ref;
				}

				NewTree.Height = OSMWay.Height * OSMToCentimetersScaleFactor;
				NewTree.BuildingLevels = OSMWay.BuildingLevels;

				NewTree.Type = OSMWay.MType;
				NewTree.BoundsMin = BoundsMin;
				NewTree.BoundsMax = BoundsMax;
	...
	for( const FOSMFile::FOSMWayInfo* OSMWay : OSMFile.Ways )
	{
		...
		else if( OSMWay->WayType == FOSMFile::EOSMWayType::Barrier )
		{
			if( AddBarrierForWay( OSMFile, *StreetMap, *OSMWay ) )
			{
				// ...
			}
		}
		else if( OSMWay->WayType == FOSMFile::EOSMWayType::Amenity )
		{
			if( AddAmenityForWay( OSMFile, *StreetMap, *OSMWay ) )
			{
				// ...
			}
		}
		else if( OSMWay->WayType == FOSMFile::EOSMWayType::Tree )
		{
			if( AddTreeForWay( OSMFile, *StreetMap, *OSMWay ) )
			{
				// ...
			}
		}
